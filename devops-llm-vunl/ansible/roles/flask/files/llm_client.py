...
class LLMClient:
    ...
    def analyze_vuln_image(self, image_file, filename: str = None) -> List[Dict]:
        ...
        try:
            ...
            **🔍 심각도 평가 기준**:
            - **높음**: 데이터 유출, 시스템 장악, 원격 코드 실행, 인증 우회 등 치명적 위험
            - **중간**: 정보 노출, 세션 관리 취약점, 입력값 검증 부족 등 중간 위험
            - **낮음**: 버전 정보 노출, 디버그 모드 활성화 등 제한적 위험
            ...
            **📊 응답 형식 (반드시 이 형식으로 응답)**:
            [
              {{
                ...
                "incidents": [
                  {{
                    "name": "발견된 취약점과 유사한 실제 해킹 사고 사례명",
                    "date": "사고 발생 날짜",
                    "summary": "발견된 취약점과 유사한 실제 해킹 사고의 상세한 내용을 5줄 이상으로 작성. 피해 규모, 영향, 대응 과정 등을 포함",
                    "source": "해킹 사고의 출처 (예: 정부 보고서, 보안 기관 발표, 언론 보도 등)"
                  }}
                ],
            ...
            **⚠️ 엄격한 요구사항**:
            ...
            - **incidents.source: 발견된 취약점과 유사한 실제 해킹 사고의 출처를 명시**
            - **risk: 최소 5줄 이상**
    ...
    def _normalize_incidents(self, incidents: Union[List, str]) -> List[Dict]:
        ...
        for i, incident in enumerate(incidents):
            ...
            if isinstance(incident, dict):
                ...
                source = incident.get('source') or incident.get('출처') or '출처: 보안진단팀 분석'
            ...
            "incidents": [
                {
                    ...
                    "source": "출처: OWASP Top 10, CWE/SANS Top 25"
                },
                {
                    "name": "웹 보안 위반 사고",
                    "date": "2024-01-01", 
